...
export default function VideoCard({
  ...
  return (
    <div
      className="group relative w-full rounded-lg bg-transparent cursor-pointer transition-all duration-300 hover:scale-[1.05] hover:z-[500]"
      onClick={handleClick}
    >
      <div className="relative aspect-[2/3] overflow-hidden rounded-lg">
        ...
        {config.showPlayButton && (
          <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100">
            <PlayCircleIcon size={50} strokeWidth={0.8} className="text-white hover:fill-green-500 hover:scale-[1.1]" />
          </div>
        )}

        <div className="absolute bottom-3 right-3 flex gap-3 opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0">
          {config.showCheckCircle && (
            <CheckCircle onClick={handleDeleteRecord} size={20} className="text-white hover:stroke-green-500" />
          )}
          {config.showHeart && (
            <Heart
              onClick={handleToggleFavorite}
              size={20}
              className={`${favorited ? 'fill-red-600 stroke-red-600' : 'fill-transparent stroke-white'} hover:scale-[1.1]`}
            />
          )}
        </div>

        {config.showRating && rate && (
          <div className="absolute top-2 right-2 bg-pink-500 text-white text-xs font-bold w-7 h-7 rounded-full flex items-center justify-center">
            {rate}
          </div>
        )}
        ...
        {config.showDoubanLink && actualDoubanId && (
          <a
            ...
            className="absolute top-2 left-2 opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0"
          >
            <div className="bg-green-500 text-white w-7 h-7 rounded-full flex items-center justify-center hover:scale-[1.1]">
              <Link size={16} />
            </div>
          </a >
        )}
      ...
      <div className="mt-2 text-center">
        <div className="relative">
          <span className="block text-sm font-semibold truncate text-gray-900 dark:text-gray-100 group-hover:text-green-600">
            {actualTitle}
          </span>
        </div>
        {config.showSourceName && source_name && (
          <span className="block text-xs text-gray-500 dark:text-gray-400 mt-1">
            <span className="inline-block border rounded px-2 py-0.5 border-gray-500/60 group-hover:border-green-500/60 group-hover:text-green-600">
              {source_name}
            </span>
          </span>
        )}
